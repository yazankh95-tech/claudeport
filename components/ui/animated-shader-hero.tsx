...
// Black shader style specifically customized for YK portfolio backgrounds
...
void main(void) {
	...
	for (float i=1.; i<12.; i++) {
		...
		vec2 p=uv;
		float d=length(p);
		// Minimal, deep black shades and shadows design
		col+=.0004/d*(cos(sin(i)*vec3(0.5, 0.5, 0.5))+0.5);
		float b=noise(i+p+bg*1.2);
		col+=.0008*b/length(max(p,vec2(b*p.x*.01,p.y)));
		col=mix(col,vec3(bg*.02, bg*.02, bg*.02),d);
	}
	O=vec4(col,1);
}`;

export const AnimatedShaderBackground: React.FC<AnimatedShaderProps> = ({ className = "" }) => {
